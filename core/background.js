chrome.runtime.onInstalled.addListener(() => {
  ...
  chrome.storage.local.get(null, async (allData) => {
    const newState = {
      ...
      mod_settings: allData.mod_settings || { enableRightClick: false, enableCopyLinkText: false, enableAdvancedPicker: false }
    };
...
chrome.contextMenus.onClicked.addListener((info, tab) => {
  ...
  if (info.menuItemId === 'copyLinkText') {
    chrome.tabs.sendMessage(tab.id, { action: 'copyLinkText' });
  }
});

function syncContextMenus() {
  chrome.storage.local.get(['mod_settings'], (res) => {
    const settings = res.mod_settings || {};
    
    // Copy Link Text
    if (settings.enableCopyLinkText) {
      chrome.contextMenus.create({
        id: 'copyLinkText',
        title: 'Copy Link Text',
        contexts: ['link']
      }, () => {
        if (chrome.runtime.lastError) { /* Ignore if already exists */ }
      });
    } else {
      chrome.contextMenus.remove('copyLinkText', () => {
        if (chrome.runtime.lastError) { /* Ignore if not found */ }
      });
    }
  });
}
...
chrome.storage.onChanged.addListener((changes) => {
  if (changes.mod_settings) {
    syncContextMenus();
    // Notify tabs to update Right-Click bypass
    chrome.tabs.query({}, (tabs) => {
      tabs.forEach(tab => {
        chrome.tabs.sendMessage(tab.id, { action: 'updateSettings', settings: changes.mod_settings.newValue }).catch(() => {});
      });
    });
  }
});
