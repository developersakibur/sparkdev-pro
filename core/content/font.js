...
function initFontFinderUI() {
  const host = document.getElementById('sd-font-finder-host');
  ...
  if (!host) {
    ...
    fontFinderHost = host;
    fontFinderShadow = host.shadowRoot;
    fontOverlay = fontFinderShadow.querySelector('.sd-overlay');
    fontTooltip = fontFinderShadow.querySelector('.sd-tooltip');
  }
}
...
function handleFontHover(e) {
  ...
  const color = (fontRgbToHex(style.color) || style.color).toUpperCase();
...
function handleFontClick(e) {
  ...
  const fontData = {
    ...
    color: (fontRgbToHex(style.color) || style.color).toUpperCase()
  };
...
function pinFontCard(data, x, y) {
  const absoluteLayer = fontFinderShadow.querySelector('#sd-absolute-layer');
...
// Internal Utility for Isolation
function fontRgbToHex(rgb) {
